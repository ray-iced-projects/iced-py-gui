...
class State:
    """global values"""
    column_widths: list[float] = [250.0, 250.0]
    column_ids: list[int] = []
    text_ids: list[int] = []
    handle_width: int = 4.0  # defaults to 4 just using for demo info
    handle_height: float = 150.0


def divider_change(div_id: int, data: tuple[int, float]):
    """Callback for when the divider is dragged"""
    idx, value = data
    # Get the difference to be added to the right column
    diff = "column_widths"[idx] - value

    # Update the left side locally and in ipg
    State.column_widths[index] = value
    update_widget(
            wid=State.column_ids[index],
            param=ColumnParam.Width,
            value=value)

    # Update the right side locally and in ipg
    if index < len(State.column_widths)-1:
        State.column_widths[index+1] += diff
        update_widget(
            wid=State.column_ids[index+1],
            param=ColumnParam.Width,
            value=State.column_widths[index+1])

    # Update the divider
    update_widget(
                wid=div_id,
                param=DividerParam.Sizes,
                value=State.column_widths)

    # Update the two text items
    update_widget(wid=State.text_ids[index],
                    param=TextParam.Content,
                    value=f"Width={value}")

    if index < len(State.column_widths)-1:
        update_widget(wid=State.text_ids[index+1],
                    param=TextParam.Content,
                    value=f"Width={State.column_widths[index+1]}")
        ...
        with Column(spacing=20.0):
            ...
            with Stack():
                ...
                with Row():
                    ...
                    for index, width in enumerate(State.column_widths):
                        # add a container for styling purposes
                        with Container(style_id=cont_style_id):
                            with Column(width=width, height=State.handle_height, spacing=10) as col:
                                State.column_ids.append(col)

                                State.text_ids.append(add_text(content=f"Width={width}"))
                ...
                add_divider(
                    ...
                    sizes=State.column_widths,
                    handle_width=State.handle_width,
                    handle_height=State.handle_height,
