    ...
    pub fn construct<'a>(
        ...
        let sh = if self.vertical_direction == Some(true) {
            let sh = SashV::new(
                ...
                .on_resize(move |id, index, size| Message::Sash(widget_id, SashMessage::Resized(id, index, size)));
...
pub fn sash_callback(state: &mut IpgState, widget_id: usize, message: SashMessage) {
    match message {
        SashMessage::Resized(_id, index, size) => {
            ...
            invoke_callback_with_args(widget_id, "on_resize", "Sash", (index, size),
                "def cb(wid: int, data: tuple[int, float])");
        },
        SashMessage::ResizedOuter(_id, size) => {
            // Fire Python callback if registered: def cb(wid: int, data: tuple[int, float])
            invoke_callback_with_args(widget_id, "on_resize_outer", "Sash", size,
...
impl WidgetParamUpdate for Sash {
    ...
    fn param_update(&mut self, param: Self::Param, value: &PyObject) {
        match param {
            ...
            SashParam::ResizeModeLastOnly => set_t_value(&mut self.resize_mode_last, value, "SashParam::ResizeModeLastOnly"),
            SashParam::ResizeModeProportional => set_t_value(&mut self.resize_mode_proportional, value, "SashParam::ResizeModeProportional"),
            SashParam::ResizeModeUniform => set_t_value(&mut self.resize_mode_uniform, value, "SashParam::ResizeModeUniform"),
