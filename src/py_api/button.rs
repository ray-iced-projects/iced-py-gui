...
/// Get length for width
fn get_width(width: Option<f32>, width_fill: bool) -> Length {
    match width {
...
/// Get length for height
fn get_height(height: Option<f32>, height_fill: bool) -> Length {
    match height {
...
/// Get padding from a vector of f64 values
fn get_padding_f64(padding: Vec<f64>) -> Padding {
    let len = padding.len();
    match len {
        0 => panic!("Padding must have a list of at least 1, 2 or 4 items"),
        1 => Padding::from(padding[0] as f32),
        2 => Padding::from([padding[0] as f32, padding[1] as f32]),
