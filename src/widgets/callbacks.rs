//! callbacks
use crate::state::{CallbackName, USERDATA1, access_callbacks, access_user_data2};
...
// ****These following functions will be depreciated when all widgets are updated
// ****The event_name will be changed to a CallbackName enum to eliminate spelling 
// or typos in the callback name.  The id and name should be unique.
// Further below are the new functions which will be renamed when updating is complete.
...
// 
// The Python callback receives: `(id,)` or `(id, user_data)` if user_data is set.
pub fn invoke_callback_enum(id: usize, event_name: CallbackName, widget_name: &str) {
    let app_cbs = access_callbacks();
...
// - Checkbox: `invoke_callback_with_args(id, "on_toggle", "Checkbox", is_checked, "def cb(wid: int, is_checked: bool)")`
// - Divider: `invoke_callback_with_args(id, "on_change", "Divider", (index, value), "def cb(wid: int, data: tuple[int, float])")`
pub fn invoke_callback_with_args_enum<A>(id: usize, event_name: CallbackName, widget_name: &str, args: A, sig_hint: &str)
where
    A: for<'py> IntoPyObject<'py> + Clone + Send,
{
    let app_cbs = access_callbacks();
...
pub fn invoke_callback_with_two_args_enum<A, B>(id: usize, event_name: CallbackName, widget_name: &str, arg1: A, arg2: B, sig_hint: &str)
where
    A: for<'py> IntoPyObject<'py> + Clone + Send,
    B: for<'py> IntoPyObject<'py> + Clone + Send,
{
