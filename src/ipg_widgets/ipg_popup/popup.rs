...
{
    pub widget: Option<Element<'a, Message, Theme, Renderer>>,
    ...
    Renderer: text::Renderer,
{
    ...
    /// Creates a new [`Popup`] without a trigger widget (controlled programmatically via `opened` parameter).
    pub fn without_widget(
        content: impl Into<Element<'a, Message, Theme, Renderer>>,
        opened: bool,
    ) -> Self {
        Popup {
    ...
    Renderer: text::Renderer,
{
    ...
    fn size(&self) -> Size<Length> {
        match &self.widget {
            Some(widget) => widget.as_widget().size(),
            None => Size::new(Length::Shrink, Length::Shrink),
        }
    }
    ...
    fn mouse_interaction(
        ...
        match &self.widget {
            Some(widget) => widget.as_widget().mouse_interaction(
                &tree.children[0],
                layout,
                cursor,
                viewport,
