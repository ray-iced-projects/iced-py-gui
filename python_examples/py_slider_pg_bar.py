...
def slider_on_change(_slider_id: int, data: float):
    update_widget(on_change_id, IpgTextParam.Content, f"On Change value is {data}")
    update_widget(bar_id, IpgProgressBarParam.Value, data)
    
def slider_on_release(_slider_id: int, data: float):
    update_widget(on_release_id, IpgTextParam.Content, f"On Release value is {data}")
...
def change_width(btn_id):
    update_widget(sl_id, IpgSliderParam.Width, 200.0)
    # change bar too
    update_widget(bar_id, IpgProgressBarParam.Width, 200.0)


def change_height(btn_id):
    update_widget(sl_id, IpgSliderParam.Height, 30.0)


def change_min(btn_id):
    update_widget(sl_id, IpgSliderParam.Min, 50.0)
    update_widget(min_txt_id, IpgTextParam.Content, "50")


def change_max(btn_id):
    update_widget(sl_id, IpgSliderParam.Max, 150.0)
    update_widget(max_txt_id, IpgTextParam.Content, "150")


def change_step(btn_id):
    update_widget(sl_id, IpgSliderParam.Step, 5.0)


def change_value(btn_id):
    update_widget(sl_id, IpgSliderParam.Value, 100.0)


def toggle_show(btn_id):
    global show
    show = not show
    update_widget(sl_id, IpgSliderParam.Show, show)
...
on_change_id = add_text(
                    parent_id="col", 
                    content=f"On Change value is 0")

on_release_id = add_text(
                    parent_id="col", 
