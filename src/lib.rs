...
use crate::py_api::combo_box::{add_combobox, add_combobox_style};
...
use crate::widgets::ipg_combo_box::{ComboBoxParam, ComboBoxStyleParam};
...
fn icedpygui(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(add_combobox_style, m)?)?;
    ...
    m.add_class::<ComboBoxStyleParam>()?;
