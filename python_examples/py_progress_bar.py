...
def change_value_plus(btn_id: int, pg_id: any):
    global value
    value += 1
    update_widget(pg_id, IpgProgressBarParam.Value, value)
    update_widget(current_value_id, IpgTextParam.Content, f"Current Value = {value}")
...
def change_min(_text_input_id: int, min_val: str, pg_id: any):
    ...
    update_widget(pg_id, IpgProgressBarParam.Min, min_float)
    # Since the text content requires a str, the min_val can be used
    update_widget(min_text, IpgTextParam.Content, min_val)


def change_max(_text_input_id: int, max_val: str, pg_id: any):
    max_float = float(max_val)
    update_widget(pg_id, IpgProgressBarParam.Max, max_float)
    update_widget(max_text, IpgTextParam.Content, max_val)


def change_height(_btn_id: int, pg_id: any):
    update_widget(pg_id, IpgProgressBarParam.Height, 30.0)


def change_width(_btn_id: int, pg_id: any):
    update_widget(pg_id, IpgProgressBarParam.Width, 300.0)


def change_width_to_fill(_btn_id: int, pg_id: any):
    update_widget(pg_id, IpgProgressBarParam.WidthFill, True)


def hide_bar(_btn_id: int, pg_id: any):
    global hide
    hide = not hide
    update_widget(pg_id, IpgProgressBarParam.Show, hide)
...
min_text = add_text("row2", "50")
sp_id = add_space("row2", width=320.0)
max_text = add_text("row2", "100")
