...
and danger.  However, you can use any of the Colors to style your widgets.  There are 155 colors
(https://www.w3schools.com/cssref/css_colors.php) to choose from or make up you own using the
rgba option.

Internally to Iced, there are 4 special color parameters used in add_styling_colors, base, strong,
weak, and text. These colors are used to color multiple parts of the widget to show mouse hover
and drag effects, etc.  These colors are calculated mostly using the background base color.
There are some where the theme primary color is used which is a predefined color for each
theme color.

To keep things simple, when you want to add colors to your widgets, IPG allows you to define
the colors you want by using terms like background_color or background_rgba.  In some
cases if you define, for example, a new background and nothing more, your background color
may be incorporated and used to define other colors for the widget using a background strong
or weak color.

This program generates the base, strong and weak colors so that you can see how the colors are used.
If you have another color you would like to add, just use get_color_palette(base_rgba=[your color])
or if you want to change the alpha on an Color by just changing the alpha, use the same function
with the alpha parameter, get_color_palette(ipg_color=your ipg color, alpha= 0.5).  The alpha
parameter is in most widgets also.
"""
...
colors = [
    ...
    Color.LAVENDER_BLUSH, Color.LAWN_GREEN, Color.LEMON_CHIFFON, Color.LIGHT_BLUE,
    Color.LIGHT_CORAL, Color.LIGHT_CYAN, Color.LIGHT_GOLDEN_ROD_YELLOW, Color.LIGHT_GRAY,
    Color.LIGHT_GREY, Color.LIGHT_GREEN, Color.LIGHT_PINK, Color.LIGHT_SALMON,
    Color.LIGHT_SEA_GREEN, Color.LIGHT_SKY_BLUE, Color.LIGHT_SLATE_GRAY, Color.LIGHT_SLATE_GREY,
    Color.LIGHT_STEEL_BLUE, Color.LIGHT_YELLOW, Color.LIME, Color.LIME_GREEN, Color.LINEN,
    Color.MAGENTA, Color.MAROON, Color.MEDIUM_AQUA_MARINE, Color.MEDIUM_BLUE,
    Color.MEDIUM_ORCHID, Color.MEDIUM_PURPLE, Color.MEDIUM_SEA_GREEN, Color.MEDIUM_SLATE_BLUE,
    Color.MEDIUM_SPRING_GREEN, Color.MEDIUM_TURQUOISE, Color.MEDIUM_VIOLET_RED,
    Color.MIDNIGHT_BLUE, Color.MINT_CREAM, Color.MISTY_ROSE, Color.MOCCASIN,
...
with Window(title="Colors", center=True, size=(550, 600)):
    ...
    with Row(width_fill=True):
        ...
        for i in range(0, 3):
            ...
            with Container(width=state["widths"][i], align_center=True):
        ...
        with Column(width_fill=True):
            ...
            for (i, color) in enumerate(colors):
                ...
                with Row():
                    ...
                    with Container(
                            style_id= style_base,
                            width=state["widths"][0], height=30.0,
                            align_center=True):
                        add_text(content=f"{state["color_name"]}")

                    add_container(
                        style_id= style_strong,
                        width=state["widths"][1],
                        height=30.0)

                    add_container(
                        style_id= style_weak,
                        width=state["widths"][2], height=30.0)
